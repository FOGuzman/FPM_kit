...
downsamp = @(x,cen) x(floor(cen(1))-floor(Np(1)/2):floor(cen(1))-floor(Np(1)/2)+Np(1)-1,...
    floor(cen(2))-floor(Np(2)/2):floor(cen(2))-floor(Np(2)/2)+Np(2)-1);
...
while abs(err1-err2)>opts.tol&&iter<opts.maxIter
    ...
    for m = 1:Nimg
        ...
        if strcmp(opts.force_phase,'on') %force phase 0 on FPM algoritm
        O = F(abs(Ft(O))); %%FORZAR FASE 0
        end
        %% position correction
